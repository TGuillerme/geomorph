---
title: "Quick Guide to Geomorph 3.0.2"
...
Progress bars are implemented in all analytical functions (print.progress = TRUE) and also `warpRefMesh`, which is to help users for long-running analyses. In this manual, progress bars are not printed, for brevity.
...
The function also returns a list containing: the PC summary (`$pc.summary`), PC scores (`$pc.scores`) -- the latter of which can be used by `plot` to reproduce the graphs for publication -- and the minimum and maximum shapes (`$pc.shapes`) for all of the PCs, which can be used in `plotRefToTarget`.
